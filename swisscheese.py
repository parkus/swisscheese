...
def circleset_area_difference(circles0,circles1,intersections=None,
                              brutegrid=False):
    """I don't know why you would want the difference, but it's not that
    hard to implement, so here you go.
    """
    ...
    if brutegrid:
        ...
        return brute_area(xrange, yrange, in_diff, brutegrid)
...
def circleset_area_subtract(circles, subcircles, intersections=None, 
    ...
    divider = len(circles)
    if intersections == None: ints = circle_intersection_pts(allcircles)
    ...
    if brutegrid:
        ...
        return brute_area(xrange, yrange, good, brutegrid)
    ...
    #above ugliness required to ensure a copy and not a copy of the pointer
    ints0 = [xpts0,xflags0,incircs0]
    Aint = circleset_area_intersect(circles,subcircles,ints)
    return circles_area_union(circles,ints0) - Aint
